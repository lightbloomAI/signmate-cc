...
function HumanBody({ config, skinColor }: { config: AvatarConfig; skinColor: THREE.Color }) {
  const clothingColor = useMemo(() => new THREE.Color(config.clothingColor), [config.clothingColor]);
  ...
  return (
    <group ref={headRef} position={[0, 0.78, 0]}>
      ...
      <group position={[0, 0.07 + expression.eyebrows * 0.015, 0.15]}>
        <mesh position={[-0.055, 0, 0]} rotation={[0, 0, expression.eyebrows * 0.15]}>
  ...
  return (
    <group ref={handRef} position={position} rotation={rotation}>
        ...
        return (
          <group key={i} position={[xPos, 0.055, 0]}>
            ...
            <mesh
              position={[
                0,
                length * 0.7 * (1 - curl * 0.5),
                curl * 0.035,
              ]}
              rotation={[curl * 1.2, 0, 0]}
            >
              <capsuleGeometry args={[width / 2 * 0.9, length * 0.35, 6, 8]} />
              <meshStandardMaterial color={skinColor} roughness={0.6} />
            </mesh>
      ...
      <group
        ...
        rotation={[shape.thumb * 0.6, isRight ? -0.4 : 0.4, isRight ? -0.3 : 0.3]}
      >
        ...
        <mesh
          position={[0, 0.065 - shape.thumb * 0.02, shape.thumb * 0.02]}
        >
...
function HumanArms({
  ...
  const [rightHandPos, setRightHandPos] = useState<[number, number, number]>([0.4, 0, 0.25]);
  const [leftHandPos, setLeftHandPos] = useState<[number, number, number]>([-0.4, 0, 0.25]);
  ...
  return (
    <>
      ...
      <group position={[0, -0.35, 0]}>
        ...
        <HumanHead config={config} expression={state.expressionState} skinColor={skinColor} />
        <HumanArms
          config={config}
...
export function AvatarRenderer({ config, state, onSignComplete, className }: AvatarRendererProps) {
  ...
  return (
    <div className={`relative ${className || ''}`} style={{ width: '100%', height: '100%', minHeight: '300px' }}>
      <Canvas
        camera={{ position: [0, 0.4, 1.6], fov: 42 }}
        shadows
      ...
      {currentSignName && !isFS && (
        <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-10">
          <div className={`px-6 py-3 rounded-xl text-white font-bold text-2xl shadow-lg transition-all duration-300 ${
      ...
      {isFS && (
        <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-10">
          <div className="bg-blue-600 px-8 py-4 rounded-2xl shadow-xl">
            <div className="text-white text-lg mb-1 text-center opacity-75">Fingerspelling</div>
      ...
      {isAnimating && (
        <div className="absolute top-4 right-4 z-10 flex items-center gap-2">
          <span className="text-white text-sm bg-gray-800/80 px-2 py-1 rounded">LIVE</span>
      ...
      {queueLength > 0 && (
        <div className="absolute bottom-4 right-4 z-10 bg-gray-800/90 px-4 py-2 rounded-lg">
          <span className="text-gray-300 text-sm font-medium">{queueLength} signs queued</span>
        </div>
      )}
      ...
      {state.currentSign && !isFS && (
        <div className="absolute bottom-4 left-4 z-10 bg-gray-900/95 px-5 py-4 rounded-xl border border-gray-700 max-w-xs">
          <div className="text-white font-bold text-xl mb-2">{state.currentSign.gloss}</div>
          <div className="text-gray-400 text-sm space-y-1">
            <div className="flex justify-between">
              <span>Hand shape:</span>
              <span className="text-gray-200">{state.currentSign.handshape.dominant}</span>
            </div>
            <div className="flex justify-between">
              <span>Movement:</span>
              <span className="text-gray-200">{state.currentSign.movement.type}</span>
            </div>
            {state.currentSign.nonManualMarkers?.length > 0 && (
              <div className="flex justify-between">
                <span>Expression:</span>
                <span className="text-gray-200">{state.currentSign.nonManualMarkers[0].expression}</span>
              </div>
            )}
          </div>
        </div>
